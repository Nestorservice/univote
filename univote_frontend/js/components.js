...
(function () {
    ...
    var Components = {
        ...
        renderEventCard: function (event) {
            ...
            var bannerUrl = event.banner_url || 'img/default-banner.jpg';
            ...
            return '<div class="bg-white p-3 feed-item rounded-4 shadow-sm mb-3">' +
                ...
                '<div class="position-relative mb-2">' +
                    '<img src="' + Utils.sanitizeHTML(bannerUrl) + '" class="img-fluid rounded-4 w-100" style="max-height:200px;object-fit:cover;background:#f0f0f0;" alt="banner">' +
                    countdownHtml +
                '</div>' +
        ...
        renderCandidatesList: function (candidates) {
            if (!candidates || candidates.length === 0) {
                return '<div class="text-center py-4 text-muted">Aucun candidat</div>';
            }
            var html = '';
            for (var i = 0; i < candidates.length; i++) {
                html += this.renderCandidateCard(candidates[i]);
            }
            return html;
        },

        renderCandidateCard: function (candidate) {
            ...
            return '<a href="candidate.html?id=' + candidate.id + '" class="text-decoration-none">' +
                '<div class="bg-white p-3 feed-item rounded-4 shadow-sm mb-3 d-flex align-items-center">' +
                    '<img src="' + Utils.sanitizeHTML(photoUrl) + '" class="img-fluid rounded-circle me-3" style="width:60px;height:60px;object-fit:cover;" alt="candidat">' +
                    '<div class="flex-grow-1">' +
                        ...
                        '<div class="mt-1"><span class="badge bg-primary bg-opacity-10 text-primary small">' + Utils.formatNumber(candidate.vote_count || 0) + ' voix</span></div>' +
                    '</div>' +
        ...
        renderCandidateDetail: function (candidate) {
            ...
            if (candidate.gallery && candidate.gallery.length > 0) {
                gallery = '<h6 class="fw-bold mt-4 mb-3">Galerie</h6><div class="row g-2 mb-3">';
                for (var i = 0; i < candidate.gallery.length; i++) {
                    gallery += '<div class="col-4"><img src="' + candidate.gallery[i] + '" class="img-fluid rounded" alt="galerie"></div>';
                }
                gallery += '</div>';
            }
            ...
            return '<div class="bg-white rounded-4 shadow-sm p-4 mb-4">' +
                '<div class="text-center mb-4">' +
                    '<img src="' + (candidate.photo_url || 'img/default-avatar.png') + '" class="img-fluid rounded-circle mb-3 border border-3 border-primary shadow" style="width:120px;height:120px;object-fit:cover;" alt="candidat">' +
        ...
        renderVideoCard: function (video, eventId) {
            ...
            return '<div class="bg-white p-3 feed-item rounded-4 shadow-sm mb-3">' +
                ...
                '<a href="video-player.html?id=' + video.id + '&event=' + eventId + '">' +
                    '<img src="' + thumbnail + '" class="img-fluid rounded mb-2 w-100" style="max-height:300px;object-fit:cover;" alt="video">' +
                '</a>' +
                '<div class="d-flex align-items-center justify-content-between">' +
                    '<div class="d-flex align-items-center">' +
                        '<button class="btn btn-link text-decoration-none like-btn" data-video="' + video.id + '">' +
                            '<span class="material-icons md-20 me-1">thumb_up_off_alt</span>' +
                            '<span>' + Utils.formatNumber(video.like_count || 0) + '</span>' +
                        '</button>' +
                        '<button class="btn btn-link text-decoration-none comment-btn" data-video="' + video.id + '">' +
                            '<span class="material-icons md-20 me-1">chat_bubble_outline</span>' +
                            '<span>' + Utils.formatNumber(video.comment_count || 0) + '</span>' +
                        '</button>' +
                    '</div>' +
        ...
        renderVideosList: function (videos, eventId) {
            if (!videos || videos.length === 0) {
                return '<div class="text-center py-4 text-muted">Aucune vidéo. Soyez le premier à uploader !</div>';
            }
            var html = '';
            for (var i = 0; i < videos.length; i++) {
                html += this.renderVideoCard(videos[i], eventId);
            }
            return html;
        },
                        ...
                        '<div class="w-100">' +
                            '<div class="d-flex align-items-center justify-content-between">' +
                                '<div class="d-flex align-items-center">' +
                                    ...
                                    '<small class="text-muted ms-2 d-none d-sm-block">@' + pseudo.toLowerCase().replace(/\s+/g, '') + '</small>' +
                                '</div>' +
                                '<div class="d-flex align-items-center small">' +
                                    '<p class="text-muted mb-0">' + timeAgo + '</p>' +
                                    '<div class="dropdown">' +
                                        '<a href="#" class="text-muted text-decoration-none material-icons ms-2 md-20 rounded-circle bg-light p-1" data-bs-toggle="dropdown">more_vert</a>' +
                                        '<ul class="dropdown-menu fs-13 dropdown-menu-end">' +
                                            '<li><a class="dropdown-item text-muted" href="#"><span class="material-icons md-13 me-1">share</span>Partager</a></li>' +
                                        '</ul>' +
                                    '</div>' +
                                '</div>' +
                            '</div>' +
                            '<div class="my-2">' +
                                ...
                                '<a href="video-player.html?id=' + videoId + '&event=' + eventId + '" class="text-decoration-none d-block position-relative">' +
                                    '<img src="' + thumbnail + '" class="img-fluid rounded-4 mb-3 w-100 border shadow-sm" style="max-height:400px;object-fit:cover;" alt="post-media">' +
                                ...
                                '<div class="d-flex align-items-center justify-content-between mb-2 px-2">' +
                                    '<div>' +
                                        '<button class="btn btn-link p-0 text-muted text-decoration-none d-flex align-items-center fw-light social-like-btn" data-video="' + videoId + '">' +
                                            '<span class="material-icons md-20 me-2">thumb_up_off_alt</span><span class="like-count">' + likes + '</span>' +
                                        '</button>' +
                                    '</div>' +
                                    '<div>' +
                                        '<button class="btn btn-link p-0 text-muted text-decoration-none d-flex align-items-center fw-light social-comment-btn" data-video="' + videoId + '">' +
                                            '<span class="material-icons md-20 me-2">chat_bubble_outline</span><span class="comment-count">' + commentsCount + '</span>' +
                                        '</button>' +
                                    '</div>' +
                                    '<div>' +
                                        '<button class="btn btn-link p-0 text-muted text-decoration-none d-flex align-items-center fw-light social-save-btn" data-video="' + videoId + '">' +
                                            '<span class="material-icons md-20 me-2">bookmark_border</span>' +
                                        '</button>' +
                                    '</div>' +
                                    '<div>' +
                                        '<a href="' + thumbnail + '" download="media_' + videoId + '.mp4" target="_blank" class="btn btn-link p-0 text-muted text-decoration-none d-flex align-items-center fw-light">' +
                                            '<span class="material-icons md-20 me-2">file_download</span>' +
                                        '</a>' +
                                    '</div>' +
                                    '<div>' +
                                        '<button class="btn btn-link p-0 text-muted text-decoration-none d-flex align-items-center fw-light" onclick="navigator.share && navigator.share({title: \'' + title.replace(/'/g, "\\'") + '\', url: window.location.origin + \'/video-player.html?id=' + videoId + '&event=' + eventId + '\'})">' +
        ...
        renderCommentsList: function (comments) {
            if (!comments || comments.length === 0) {
                return '<div class="text-center py-3 text-muted">Aucun commentaire</div>';
            }
            var html = '';
            for (var i = 0; i < comments.length; i++) {
                html += this.renderComment(comments[i]);
            }
            return html;
        },
        ...
        renderResultsBar: function (candidate, totalVotes) {
            var percent = 0;
            if (totalVotes > 0) {
                percent = Math.round((candidate.vote_count / totalVotes) * 100);
            }
            var photoUrl = candidate.photo_url || 'img/default-avatar.png';
        ...
        showLoading: function () {
            return '<div class="text-center py-5">' +
                ...
                '</div>' +
                '<p class="mb-0 text-muted">Chargement...</p>' +
                '</div>';
        },
        ...
        showEmpty: function (message) {
            return '<div class="text-center py-4 text-muted">' +
                ...
                '<p class="mb-0">' + Utils.sanitizeHTML(message || 'Aucun contenu') + '</p>' +
                '</div>';
        }
    };
